...
Search the SL index before capture. Capture only verified reusable evidence.
An agent or host must run
...
candidate. Run the committed runtime's `project` and `validate` commands for
every change. Outcome association is evidence, not proof that the guidance
