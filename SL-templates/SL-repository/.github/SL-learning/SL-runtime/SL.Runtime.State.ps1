...
$script:SLLegacyRegistryPath = '.github/SL-learning/SL-registry.json'
$script:SLLegacyIndexPath = '.github/SL-learning/SL-index.json'
...
function Get-SLRegistry {
    ...
    $Artifacts = @{}
    $LegacyIds = @{}
    $LegacyPath = Resolve-SLContainedPath -Root $Root -RelativePath $script:SLLegacyRegistryPath
    if ([IO.File]::Exists($LegacyPath)) {
        $Legacy = Read-SLJson -Root $Root -RelativePath $script:SLLegacyRegistryPath
        foreach ($Artifact in @($Legacy.artifacts)) {
            if ($LegacyIds.ContainsKey([string] $Artifact.id)) {
                Throw-SLContractError -Code 'registry-duplicate' -Message "Duplicate artifact ID $($Artifact.id) in legacy registry."
            }
            $LegacyIds[[string] $Artifact.id] = $true
            Set-SLProperty $Artifact 'scope' (Normalize-SLScope (Get-SLProperty $Artifact 'scope' $script:SLDefaultScope))
            $Artifacts[[string] $Artifact.id] = $Artifact
        }
    }
    $Owners = @{}
    $Catalog = Get-SLStateCatalog $Root
    $RegistryPaths = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($Entry in @($Catalog.scopes)) {
        [void] $RegistryPaths.Add([string] $Entry.registryPath)
    }
    $ScopeRootPath = Resolve-SLContainedPath -Root $Root -RelativePath $script:SLScopeRoot
    if ([IO.Directory]::Exists($ScopeRootPath)) {
        foreach ($File in [IO.Directory]::EnumerateFiles($ScopeRootPath, 'SL-registry.json', [IO.SearchOption]::AllDirectories)) {
            [void] $RegistryPaths.Add([IO.Path]::GetRelativePath($Root, $File).Replace('\', '/'))
        }
    }
    foreach ($RegistryRelativePath in (Sort-SLOrdinal @($RegistryPaths))) {
        ...
        foreach ($Artifact in @($Shard.artifacts)) {
            ...
            if ($LegacyIds.ContainsKey([string] $Artifact.id)) {
                $LegacyIdentity = ConvertTo-SLCanonicalJson (Get-SLArtifactIdentity $Artifacts[[string] $Artifact.id])
                $ShardIdentity = ConvertTo-SLCanonicalJson (Get-SLArtifactIdentity $Artifact)
                if ($LegacyIdentity -cne $ShardIdentity) {
                    Throw-SLContractError -Code 'registry-overlay' -Message "Conflicting legacy registry/shard overlay for artifact $($Artifact.id)."
                }
            }
            $Artifacts[[string] $Artifact.id] = $Artifact
        }
